...
var configSchema = exports_external.object({
  ...
  ssaFeeds: exports_external.array(exports_external.object({
    name: exports_external.string(),
    feedContract: exports_external.string()
  }))
});
var safeJsonStringify = (obj) => JSON.stringify(obj, (_, value2) => typeof value2 === "bigint" ? value2.toString() : value2, 2);
function readSSAFeed(runtime2, evmClient, name, address) {
  try {
    runtime2.log(`Reading S&P SSA feed: ${name} at ${address}`);
    ...
    runtime2.log(`✓ ${name}: SSA Rating = ${rating}`);
    return {
      name,
      address,
      rating
    ...
    runtime2.log(`✗ ${name}: Failed - ${error}`);
    return {
      name,
      address,
      rating: null,
...
var onCronTrigger = (runtime2, payload) => {
  ...
  runtime2.log("=== S&P SSA Rating Refresh Workflow ===");
  runtime2.log(`Chain: ${runtime2.config.chainSelectorName}`);
  runtime2.log(`Feeds to read: ${runtime2.config.ssaFeeds.length}`);
  ...
  const results = runtime2.config.ssaFeeds.map((feed) => readSSAFeed(runtime2, evmClient, feed.name, feed.feedContract));
  runtime2.log("=== Workflow Complete ===");
